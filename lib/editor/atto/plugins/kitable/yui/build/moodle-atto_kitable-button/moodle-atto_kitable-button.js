YUI.add('moodle-atto_kitable-button', function (Y, NAME) {
...
/**
 * @module moodle-atto_kitable-button
 */

/**
 * Atto text editor table plugin.
 *
 * @namespace M.atto_kitable
 * @class Button
 * @extends M.editor_atto.EditorPlugin
 */

var COMPONENT = 'atto_kitable',
    EDITTEMPLATE = '' +
        '<form class="{{CSS.FORM}}">' +
            '<label for="{{elementid}}_atto_kitable_caption">{{get_string "caption" component}}</label>' +
            '<input class="{{CSS.CAPTION}} fullwidth" id="{{elementid}}_atto_kitable_caption" required />' +
            '<br/>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_kitable_headers" class="sameline">{{get_string "headers" component}}</label>' +
            '<select class="{{CSS.HEADERS}}" id="{{elementid}}_atto_kitable_headers">' +
            ...
            '<br/>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_kitable_tabletype" class="sameline">{{get_string "tabletype" component}}</label>' +
            '<select class="{{CSS.TABLETYPE}}" id="{{elementid}}_atto_kitable_tabletype">' +
                '<option value="kitable1">{{get_string "table1" component}}' + '</option>' +
                '<option value="kitable2 table-no-lines">{{get_string "table2nolines" component}}' + '</option>' +
                '<option value="kitable2 table-lines">{{get_string "table2lines" component}}' + '</option>' +
                '<option value="kitable3">{{get_string "table3" component}}' + '</option>' +
            '</select>' +
            '<div class="mdl-align">' +
                '<br/>' +
    ...
    TEMPLATE = '' +
        '<form class="{{CSS.FORM}}">' +
            ...
            '</select>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_kitable_rows" class="sameline">{{get_string "numberofrows" component}}</label>' +
            '<input class="{{CSS.ROWS}}" type="number" value="3" id="{{elementid}}_atto_kitable_rows" size="8" min="1" max="50"/>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_kitable_columns" class="sameline">{{get_string "numberofcolumns" component}}</label>' +
            '<input class="{{CSS.COLUMNS}}" type="number" value="3" id="{{elementid}}_atto_kitable_columns" size="8" min="1" max="20"/>' +
            '<br/>' +
            '<br/>' +
            '<label for="{{elementid}}_atto_kitable_tabletype" class="sameline">{{get_string "tabletype" component}}</label>' +
            '<select class="{{CSS.TABLETYPE}}" id="{{elementid}}_atto_kitable_tabletype">' +
    ...
    SELECTORS = {
        ...
        ROWS: '.' + CSS.ROWS,
        COLUMNS: '.' + CSS.COLUMNS,
        TABLETYPE: '.' + CSS.TABLETYPE,
        SUBMIT: '.' + CSS.SUBMIT,
        FORM: '.' + CSS.FORM
    };

Y.namespace('M.atto_kitable').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _updateTable: function(e) {
        ...
        tabletype = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.TABLETYPE);

        table = this._lastTarget.ancestor('table');

        table.setAttribute('class', tabletype.get('value'));
    ...
    _setTable: function(e) {
        ...
        var nl = "\n";
        // start change uh 2014-11-25
        //tablehtml = '<br/>' + nl + '<table>' + nl;
        tablehtml = '<br/>' + nl + '<table class="' + tabletype.get('value') + '">' + nl;
        // end change uh
    ...
    _showTableMenu: function(e) {
        ...
        // start change uh 2014-11-25
        // Comment - automate the reference to the button: this.buttons.kitable
        // It took me some time to find this place which I needed to change
        // after I duplicated the table plugin
        // end change uh
        this._contextMenu.align(this.buttons.kitable, [Y.WidgetPositionAlign.TL, Y.WidgetPositionAlign.BL]);
    ...
    _editTable: function() {
        ...
        var node = this._getEditDialogueContent(),
            ...
            tabletypeinput = node.one(SELECTORS.TABLETYPE),
            table = this._lastTarget.ancestor('table'),
            captionnode = table.one('caption');
        ...
        tabletypeinput.set('value', table.getAttribute('class'));
