...
class Report {
    ...
    public function prepare_and_send_pdf() {
        try {
            ...
            $result = get_string('error_creating_pdf','report_generator');
            return $result;
        }//try_create_pdf error_download_pdf


        $report_format = $this->get_report_format();
        if( $report_format == REPORT_GENERATOR_REP_FORMAT_PDF_MAIL ) {
            if ($this->save_pdf_to_send()) {
                $result = $this->send_email_pdf();
                $result = $this->create_send_pdf_dialog($result);
                return $result;
            }else {
                $result = get_string('send_pdf_file_error_dialog_text','report_generator');
                return $result;
            }
        }else {
            try {
                ...
                $result = get_string('error_download_pdf','report_generator');
                return $result;
            }//try_download_pdf
        }//if_report_format
    ...
    private function send_email_pdf () {
        ...
        if(file_exists($file_path . '/' . $this->file_name)) {
            ...
            return get_string('send_pdf_file_error_dialog_text','report_generator');
        }
    }//send_email_pdf
    ...
    public static function get_string( $name, $a=null ) {
        return get_string( $name, 'report_generator', $a );
    }//get_string
    ...
    private function print_pdf_tables_course(&$pdf,$table) {
        ...
        /* Completed*/
        if ($table[REPORT_GENERATOR_COMPLETED]) {
            $completed          = $table[REPORT_GENERATOR_COMPLETED];
        ...
        /* Progress */
        if ($table[REPORT_GENERATOR_IN_PROGRESS]) {
            $progress          = $table[REPORT_GENERATOR_IN_PROGRESS];
        ...
        /* Before */
        if ($table[REPORT_GENERATOR_COMPLETED_BEFORE]) {
            $before            = $table[REPORT_GENERATOR_COMPLETED_BEFORE];
