 ...
 */
function report_manager_get_company_name($company) {
    global $DB;
...
}//report_manager_get_company_name
 ...
 */
function report_manager_get_job_role_list($list = null){
    /* Variables    */
    global $DB;
    $job_role_list = array();
 ...
 */
function report_manager_get_job_role_name($job_role_id) {
    /* Variables    */
    global $DB;
...
}//report_manager_get_job_role_name
 ...
 */
function report_manager_get_course_list() {
    /* Variables    */
    global $DB;
 ...
 * Report format List
 */
function report_manager_get_report_format_list( $with_csv = true ) {
    $list = array(
        REPORT_MANAGER_REP_FORMAT_SCREEN      => get_string('preview', 'report_manager'),
        REPORT_MANAGER_REP_FORMAT_PDF         => get_string('pdf', 'report_manager'),
        REPORT_MANAGER_REP_FORMAT_PDF_MAIL    => get_string('pdf_and_email', 'report_manager')
    );

    if( $with_csv )
    {
        $list[ REPORT_MANAGER_REP_FORMAT_CSV ] = get_string('export_csv', 'report_manager');
    }
 ...
 */
function report_manager_get_companies_to_report($data_form) {
    /* Variables */
    $company_list = array();
    ...
    switch ($report_level) {
        case 1:
            $parent_list = report_manager_get_company_level(2,$data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'1']);
            if (!$parent_list) {
                return false;
            }//if_empty
            $parent_list = join(',',array_keys($parent_list));
            $company_list = report_manager_get_company_level(3,$parent_list);
            break;
        case 2:
            $company_list = report_manager_get_company_level(3,$data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'2']);
            if (!$company_list) {
                return false;
            }//if_empty
            break;
        case 3:
            $company_list = report_manager_get_company_level(3,$data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'2']);
            if (!empty($data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'3'])) {
                $company_keys   = array_keys($company_list);
                $companies      = array_intersect($data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'3'],$company_keys);
    ...
    return $company_list;
}//report_manager_get_companies_to_report
 ...
 */
function report_manager_display_course_report($data_form){
    global $DB;

    /* Variables */
    $return_url = new moodle_url('/report/manager/course_report/course_report_level.php',array('rpt' => $data_form['rpt']));
    $return     = '<a href="'.$return_url .'">'. get_string('course_return_to_selection','report_manager') .'</a>';
    $no_data    = get_string('no_data', 'report_manager');
    $no_data   .=  '<br/>' . $return;
    $data       = array();
    $out        = '';

    /* Get Information about course  */
    $course = $DB->get_record('course',array('id'=>$data_form[REPORT_MANAGER_COURSE_LIST]));
    /* All job roles  selected */
    if (!empty($data_form[REPORT_MANAGER_JOB_ROLE_LIST])) {
        $list = join(',',$data_form[REPORT_MANAGER_JOB_ROLE_LIST]);
        $job_role_list = report_manager_job_role_outcome_course_list($list);
    }else {
        $job_role_list = report_manager_job_role_outcome_course_list();
    }//if_else
    ...
    $completed_time = report_manager_get_completed_date_timestamp($completed_time,true);

    /* Get companies selected */
    $course_report_info = report_manager_get_companies_to_report($data_form);
    ...
    report_manager_get_course_report_info($course,$course_report_info,$job_role_list,$completed_time);
    if (!$course_report_info) {
        return $no_data;
    }
    ...
    $company_id                 = $data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'1'];
    $data['level_1']            = report_manager_get_company_name($company_id);

    if ($data['report_level'] > 1) {
        $company_id             = $data_form[REPORT_MANAGER_COMPANY_STRUCTURE_LEVEL .'2'];
        $data['level_2']        = report_manager_get_company_name($company_id);
    }
    $data['format']             = $data_form[REPORT_MANAGER_REPORT_FORMAT_LIST];

    /* Create Data */
    switch ($data_form[REPORT_MANAGER_REPORT_FORMAT_LIST]) {
        case REPORT_MANAGER_REP_FORMAT_SCREEN:
            $out = '<br/><br/>';
            $out .= report_manager_create_course_report_screen_out($data);
            $out .= $return;
            $out .= '<br/><br/>';
            break;
        case REPORT_MANAGER_REP_FORMAT_PDF:
        case REPORT_MANAGER_REP_FORMAT_PDF_MAIL:
            $out = report_manager_create_course_report_pdf_out($data);
            ...
            break;
        case REPORT_MANAGER_REP_FORMAT_CSV:
            $out = report_manager_create_course_report_csv_out($data);
           break;
    }//switch_report

    return $out;
}//report_manager_display_course_report
 ...
 *                                                          --> time_completed
 */
function report_manager_get_course_report_info($course,&$company_list,&$job_role_list,$completed_time) {
    global $DB;
        ...
        /* Execute  */
            ...
            if ($rdo_users) {
                foreach ($rdo_users as $user) {
                    ...
                    $time_completed             = report_manager_get_course_completed_info_user($params['course_id'],$user->id);
...
}//report_manager_get_course_report_info
 ...
 */
function report_manager_job_role_outcome_course_list($list = null) {
    global $DB;
    ...
    return $job_role_list;
}//report_manager_job_role_outcome_course_list
 ...
 */
function report_manager_get_course_completed_info_user($course_id,$user_id) {
    global $DB;
...
}//report_manager_get_course_completed_info_user
 ...
 */
function report_manager_get_cvs_outcome_table($empty_row,$report_level,$company,$report_info,$type,&$out_cvs) {
        ...
        foreach($report_info as $job=>$info) {
            if ($info->courses) {
                foreach ($info->courses as $co=>$course) {
                    $users = report_manager_get_users_info_table($type,$course);
                    if ($users) {
                        $row        = $empty_row;
                        $row[0]     = $company;
                        $row[1]     = $info->job_role;
...
}//report_manager_get_cvs_outcome_table
 ...
 * Create a empty row.
 */
function report_manager_create_empty_row_cvs($cols_cvs) {
    $empty_row = array();
    ...
    return $empty_row;
}//report_manager_create_empty_row_cvs
 ...
 */
function report_manager_download_report_cvs($report,$report_type) {
    global $CFG;
    ...
    fclose($fh);
    exit;
}//report_manager_download_report_cvs
 ...
 */
function report_manager_create_course_report_csv_out($data) {
    /* Variables    */
    ...
    $fields_csv = report_manager_get_fields_report_course_csv($report_level);
    /* Create Empty Row */
    $cols_cvs   = count($fields_csv);
    $empty_row  = report_manager_create_empty_row_cvs($cols_cvs);

    /* Create Header    */
    $out_csv = report_manager_get_header_report_course_csv($empty_row,$report_level,$course,$level_1,$level_2);
    /* Fill the file    */
    report_manager_print_report_course_csv_tables($empty_row,$report_type,$report_level,$course_report_info,$completed_time,$out_csv);
    /* Download the CVS report  */
    report_manager_download_report_cvs($out_csv,$report_type);

    return true;
}//report_manager_create_course_report_csv_out
 ...
 */
function report_manager_get_fields_report_course_csv($report_level) {
    /* Variables    */
    $str_job_role       = get_string('job_role', 'report_manager');
    $str_count          = get_string('count', 'report_manager');
    $str_company_name   = get_string('company', 'report_manager');
    $str_username       = get_string('name');
    $str_cert_date      = get_string('cert_date', 'report_manager');
    ...
    return $fields_csv;
}//report_manager_get_fields_report_course_csv
 ...
 */
function report_manager_get_header_report_course_csv($empty_row,$report_level,$course,$level_1,$level_2) {
    /* Data of the report   */
    $out = array();
    ...
    /* Level 1 */
    $row    = $empty_row;
    $row[0] = get_string('company_structure_level', 'report_manager', 1) . ': ' . $level_1;
    $out[]  = $row;
    /* Level 2 */
    if ($report_level > 1) {
        $row    = $empty_row;
        $row[0] = get_string('company_structure_level', 'report_manager', 2) . ': ' . $level_2;
        $out[]  = $row;
    }//if_level_2

    return $out;
}//report_manager_get_header_report_course_csv
 ...
 */
function report_manager_print_report_course_csv_tables($empty_row,$report_type,$report_level,$course_report_info,$completed_time,&$out_csv) {
    /* Variables     */
    $completed_time = userdate($completed_time,'%d.%m.%Y', 99, false);
    $str_completed  = get_string($report_type . '_units_have_completed_since', 'report_manager',$completed_time);
    $str_progress   = get_string($report_type . '_units_in_progress', 'report_manager');
    $str_before     = get_string($report_type . '_units_have_completed_before', 'report_manager',$completed_time);
    ...
    foreach ($course_report_info as $id=>$company_info) {
        ...
        if ($company_info->total_completed) {;
            ...
            $out_csv[]  = $row;
            report_manager_get_cvs_course_table($empty_row,$report_level,$company,$report_job,$total,'completed',$out_csv);
        }//_completed
        ...
        if ($company_info->total_progress) {
            ...
            $out_csv[]  = $row;
            report_manager_get_cvs_course_table($empty_row,$report_level,$company,$report_job,$total,'progress',$out_csv);
        }//_progress
        ...
        if ($company_info->total_before) {
            ...
            $out_csv[]  = $row;
            report_manager_get_cvs_course_table($empty_row,$report_level,$company,$report_job,$total,'before',$out_csv);
        }//_before
    }//for_company
}//report_manager_print_report_course_csv_tables
 ...
 * Get the data for the csv course file
 */
function report_manager_get_cvs_course_table($empty_row,$report_level,$company,$report_info,$total,$type,&$out_csv) {
    switch($report_level) {
        case 1:case 2;
            foreach($report_info as $job=>$info) {
                $users       = report_manager_get_users_info_table($type,$info);
                if ($users) {
                    $row        = $empty_row;
                    $row[0]     = $company;
                    $row[1]     = $info->job_role;
...
}//report_manager_get_cvs_course_table
 ...
 */
function report_manager_create_course_report_screen_out($data) {
    /* Variables */
    $out                = '';
    $report_type        = $data['report_type'];
    $report_level       = $data['report_level'];
    ...
    $out .= '<ul class="level-list unlist">';
        $out .= '<li>';
            $out .= '<h2>';
            $out .= get_string('company_structure_level', 'report_manager', 1) . ': ' . $level_1;
            $out .= '</h2>';
        $out .= '</li>';
        ...
        if ($report_level >1) {
            $out .= '<li>';
                $out .= '<h2>';
                $out .= get_string('company_structure_level', 'report_manager', 2) . ': ' . $level_2;
                $out .= '</h2>';
            $out .= '</li>';
        }//if_level_2
    $out .= '</ul>';

    $out .= report_manager_print_report_tables($report_type,$report_level,$course_report_info,$completed_time);

    return $out;
}//report_manager_create_course_report_screen_out
 ...
 */
function report_manager_create_course_report_pdf_out($data) {
    /* Variables    */
    $out_pdf                = array();
    $report_type            = $data['report_type'];
    $report_level           = $data['report_level'];
    ...
    /* Information about Companies Level*/
    $out_pdf['level_1'] = get_string('company_structure_level', 'report_manager', 1) . ': ' . $level_1;
    if ($report_level > 1) {
        $out_pdf['level_2'] = get_string('company_structure_level', 'report_manager', 2) . ': ' . $level_2;
    }//if_level_2
    ...
    report_manager_print_report_pdf_tables($report_type,$report_level,$course_report_info,$completed_time,$out_pdf);
    return report_manager_prepare_send_pdf($out_pdf);
}//report_manager_create_course_report_pdf_out
 ...
 */
function report_manager_prepare_send_pdf($report_data) {

    $report = new report($report_data);

    return $report->prepare_and_send_pdf();
}//report_manager_prepare_send_pdf
 ...
 */
function report_manager_print_report_pdf_tables($report_type,$report_level,$company_report_info,$completed_time,&$out_pdf) {
    /* Variables     */
    $completed_time = userdate($completed_time,'%d.%m.%Y', 99, false);
    $str_completed  = get_string($report_type . '_units_have_completed_since', 'report_manager',$completed_time);
    $str_progress   = get_string($report_type . '_units_in_progress', 'report_manager');
    ...
    foreach ($company_report_info as $id=>$company_info) {
        ...
        if ($company_info->total_completed) {
            ...
            $table_completed = report_manager_get_table($report_type,$report_level, 'completed',$company,$company_info->report_job,$company_info->total_completed);
        ...
        if ($company_info->total_progress) {
            ...
            $table_progress = report_manager_get_table($report_type,$report_level, 'progress',$company,$company_info->report_job,$company_info->total_progress);
        ...
        if ($company_info->total_before) {
            ...
            $table_before = report_manager_get_table($report_type,$report_level,'before',$company,$company_info->report_job,$company_info->total_before);
            $out_before['tables'] = $table_before;
        }//_before

        $out[$id][REPORT_MANAGER_COMPLETED]        = $out_completed;
        $out[$id][REPORT_MANAGER_IN_PROGRESS]      = $out_progress;
        $out[$id][REPORT_MANAGER_COMPLETED_BEFORE] = $out_before;
    }//for_company_list

    $out_pdf['tables'] = $out;
}//report_manager_print_report_pdf_tables
 ...
 * Create the tables that contains all information to the report.
 */
function report_manager_print_report_tables($report_type,$report_level,$company_report_info,$completed_time) {
    /* Variables     */
    $completed_time = userdate($completed_time,'%d.%m.%Y', 99, false);
    $str_completed  = get_string($report_type . '_units_have_completed_since', 'report_manager',$completed_time);
    $str_progress   = get_string($report_type . '_units_in_progress', 'report_manager');
    ...
    return $out;
}//report_manager_print_course_report_tables
 ...
 */
function report_manager_get_head_course_table($report_level,$company,&$table) {
    $str_job_role       = get_string('job_role', 'report_manager');
    $str_count          = get_string('count', 'report_manager');
    $str_company_name   = get_string('company', 'report_manager');
    $str_username       = get_string('name');
...
}//report_manager_get_head_course_table
 ...
 */
function report_manager_get_head_outcome_table($report_level,$company,&$table) {
    $str_job_role       = get_string('job_role', 'report_manager');
    $str_count          = get_string('count', 'report_manager');
    $str_company_name   = get_string('company', 'report_manager');
...
}//report_manager_get_head_outcome_table
 ...
 */
function report_manager_add_row_course_table($report_level,$job,$row_info,&$table) {
    switch ($report_level) {
        case 1; case 2:
            /* Rows */
            $row = array();
...
}//report_manager_add_row_course_table
 ...
 */
function report_manager_add_row_outcome_table($report_level,$row_info,$type,&$table){
    switch ($report_level){
        case 1;case 2:
            /* Rows */
            $row = array();
        ...
        case 3:
            ...
            /* Add Last row */
            $table->data[] = report_manager_add_last_row_outcome_table($total_users);
            break;
    }//switch
}//report_manager_add_row_outcome_table
 ...
 */
function report_manager_add_last_row_outcome_table($total){
    /* Rows */
    $row = array();
    ...
    return $row;
}//report_manager_add_last_row_outcome_report
 ...
 */
function report_manager_get_users_info_table($type,$info) {
    $users = array();
    ...
    return $users;
}//report_manager_get_users_info_table
 ...
 */
function report_manager_add_last_row_course_table($total, &$table) {
    /* First Row */
    $row = array();
    /* Company Col  */
    $row[] = '';
    ...
    $row[] = $total;
    $table->data[] = $row;
}//report_manager_add_last_row_course_table
 ...
 */
function report_manager_get_table($report_type,$report_level,$type_table,$company,$report_info,$total) {
    /* Create Table */
    $table = new html_table();

    switch ($report_type) {
        case 'course':
            report_manager_get_head_course_table($report_level,$company,$table);
            /* Row -> Job Roles */
            foreach ($report_info as $id=>$info) {
                $users = report_manager_get_users_info_table($type_table,$info);

                if ($users) {
                    report_manager_add_row_course_table($report_level,$info->job_role,$users,$table);
                }//if_users
            }//for
            if ($report_level == 3) {
                report_manager_add_last_row_course_table($total, $table);
            }
            break;
        case 'outcome':
            report_manager_get_head_outcome_table($report_level,$company,$table);
            foreach($report_info as $id=>$info) {
                if ($info->courses) {
                    report_manager_add_row_outcome_table($report_level,$info,$type_table,$table);
                }//if_courses
            }//for
            break;
    }//switch_report_type
 ...
 * @return      string
 */
function report_manager_get_feedback_dialog($text) {
    $sendpdfdialog = '<div class="modal" id="sendpdfdialog" title="' .
        get_string( 'sendpdfdialogtitle', 'report_reportmanager' ) . '">';
    $sendpdfdialog .= '<div>';
 ...
 */
function report_manager_print_report_page($tab, $site_context) {
    global $OUTPUT;
    ...
    $out = '<ul class="unlist report-selection">' . "\n";
        if (has_capability('report/manager:viewlevel3', $site_context)) {
            $out .= report_manager_get_links_report_third_level($tab);
        }else if(has_capability('report/manager:viewlevel2', $site_context)) {
            //$out .= report_manager_get_links_report_second_level($tab);
        }else if (has_capability('report/manager:viewlevel1', $site_context)) {
            //$out .= report_manager_get_links_report_first_level($tab);
        }//if_capabitity
    $out .= '</ul>' . "\n";

    /* Draw Links */
    echo $OUTPUT->heading($out);
}//report_manager_print_course_report_page
 ...
 */
function report_manager_get_links_report_first_level($tab) {
    $url_level_1 = new moodle_url('/report/manager/' . $tab .'/' . $tab .'_level.php',array('rpt'=>1));

    $out = '';
    $out .= '<li class="first">' . "\n";
    $out .= '<a href="'.$url_level_1 .'">'. get_string('level_report','report_manager',1) .'</a>';
    $out .= '</li>' . "\n";

    return $out;
}//report_manager_get_links_report_first_level
 ...
 */
function report_manager_get_links_report_second_level($tab) {
    $url_level_2 = new moodle_url('/report/manager/' . $tab .'/' . $tab .'_level.php',array('rpt'=>2));

    $out = report_manager_get_links_report_first_level($tab);
    $out .= '<li>' . "\n";
    $out .= '<a href="'.$url_level_2 .'">'. get_string('level_report','report_manager',2) .'</a>';
    $out .= '</li>' . "\n";

    return $out;
}//report_manager_get_links_report_second_level
 ...
 */
function report_manager_get_links_report_third_level($tab) {
    $url_level_3 = new moodle_url('/report/manager/' . $tab .'/' . $tab .'_level.php',array('rpt'=>3));

    //$out = report_manager_get_links_report_second_level($tab);
    $out = '<li class="last">' . "\n";
    $out .= '<a href="'.$url_level_3 .'">'. get_string('level_report','report_manager',3) .'</a>';
    $out .= '</li>' . "\n";

    return $out;
}//report_manager_get_links_report_third_level
