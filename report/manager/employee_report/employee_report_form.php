...
class manager_employee_report_form extends moodleform {
    function  definition(){
        ...
        $form->addElement('html', '<div class="level-wrapper">');
                $outcome_lst = report_manager_EmployeeReport_getOutcomes();

            $form->addElement('select',REPORT_MANAGER_OUTCOME_LIST,get_string('select_outcome_to_report', 'report_manager'),$outcome_lst);
            $form->addRule(REPORT_MANAGER_OUTCOME_LIST, null, 'required');

            $form->addElement('select',REPORT_MANAGER_COMPLETED_LIST,get_string('expired_next', 'report_manager'),report_manager_get_completed_list());
    ...
    function validation($data, $files) {
        ...
        if (!$data[REPORT_MANAGER_OUTCOME_LIST]){
            $errors[REPORT_MANAGER_OUTCOME_LIST] = get_string('required');
        }//if_outcome_list
