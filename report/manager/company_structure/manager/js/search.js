...
/**
 * Report Competence Manager - Manager Javascript
 *
 * Description
 *
 * @package         report/manager
 * @subpackage      company_structure/manager/js
...
M.core_user.manager_selectors = [];

M.core_user.get_manager_selector = function (name) {
...
M.core_user.init_managers_selector = function (Y, name, hash, level, companies, lastsearch) {
    var manager_selector = {
        ...
        output_options : function(data) {
            ...
            this.listbox.all('optgroup').each(function(optgroup){
                optgroup.all('option').each(function(option){
                    if (option.get('selected')) {
                        selected_managers[option.get('value')] = {
                            id : option.get('value'),
            ...
            for (var key in data.results) {
                ...
                this.output_group(groupdata.name, groupdata.users, selected_managers, true);
                count ++;
            }

            if (!count) {
                var searchstr = (this.lastsearch != '') ? this.insert_search_into_str(M.str.moodle.nomatchingusers, this.lastsearch) : M.str.moodle.none;
                this.output_group(searchstr, {}, selected_managers, true)
            }

            // If there were previously selected users who do not match the search, show them too.
            if (this.get_option('preserveselected') && selected_managers) {
                this.output_group(this.insert_search_into_str(M.str.moodle.previouslyselectedusers, this.lastsearch), selected_managers, true, false);
            }
        ...
        output_group : function(groupname, managers, selected_managers, processsingle) {
            ...
            for (var key in managers) {
                ...
                if (manager.infobelow) {
                    ...
                    extraoption.appendChild(document.createTextNode(manager.infobelow));
            ...
            this.listbox.get("options").each( function() {
                if (selected_managers[this.get('value')]) {
    ...
    this.manager_selectors[name] = manager_selector;
