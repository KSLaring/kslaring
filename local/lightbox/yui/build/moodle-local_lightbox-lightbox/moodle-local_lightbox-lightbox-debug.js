    ...
    hideLightBox: function () {
        ...
        if (scoObject) {
            ...
            if (scoDocument.all.length &&
                ...
                Y.log('hideLightBox - SCORM GENERATED BY:  Lectora');
                var cframe = scoDocument.getElementsByName('contentframe')[0];

                if (cframe) {
                    fw = cframe.contentWindow;
                    Y.log('hideLightBox - content window found ...');

                    if (fw) {
                        var $fw = Y.one(fw);
                        $fw.on('unload', function () {
                            Y.log('SCORM-window unload triggered');
                        });
                        $fw.on('beforeunload', function () {
                            Y.log('SCORM-window beforeunload triggered');
                        });
                        ...
                        if ('finish' in fw) {
                            if (fw.is.webkit) {
                                ...
                                Y.log('hideLightBox - Bind finish to onunload.');
                            }
                        }

                        Y.log('finishCalled:', fw.finishCalled, 'autoCommit:', fw.autoCommit);

                        if ('doQuit' in fw) {
                            Y.log('Change the doQuit function ...');
                            fw.doQuit_orig = fw.doQuit;
                            fw.doQuit = function doQuit(bForce) {
                                Y.log('Lectora called doQuit - call function with changed parameter ...');
                                bForce = 0;
                                fw.doQuit_orig(bForce);
                            };
                        } else if (fw.finishCalled !== true) {
                            Y.log('Force autoCommit');
