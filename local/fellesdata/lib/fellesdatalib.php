...
class FSKS_COMPANY {
     ...
     * @author          eFaktor     (fbv)
     */
    public static function update_company_changes_same_level() {
        /* Variables */
        global $DB;
        $rdo        = null;
        $sql        = null;
    ...
    private static function synchronize_company_ksfs($companyKSFS,$impKey) {
        ...
        try {
            ...
            // Synchronized
            if ($sync) {
                $instance = $DB->get_record('fs_imp_company',array('org_enhet_id' => $impKey,'action' => $companyKSFS->action),'id,imported');
                if ($instance) {
                    $instance->imported         = 1;
                    $instance->timemodified     = $time;
                    $DB->update_record('fs_imp_company',$instance);
