...
/**
 * Fellesdata Integration - Javascript Jobroles search
 *
...
M.core_user.search_jobroles = [];

M.core_user.get_search_jobrole = function (name) {
    return this.search_jobroles[name] || null;
};

M.core_user.init_search_jobrole = function (Y, name, jobrole, hash, lastsearch) {
    var search_jobrole = {
        ...
        handle_selection_change : function() {
            ...
            if (isselectionempty !== this.selectionempty) {
                this.fire('search_jobrole:selectionchanged', isselectionempty);
            }
        ...
        send_query : function(forceresearch) {
            ...
            var ksJR = this.ks_jobrole.get('value');
            var iotrans = Y.io(M.cfg.wwwroot + '/local/fellesdata/mapping/searchjr.php', {
                method: 'POST',
                data: 'ks_jobrole=' + ksJR + '&search=' + value + '&selectorid=' + hash + '&sesskey=' + M.cfg.sesskey,
                on: {
        ...
        output_options : function(data) {
            ...
            this.listbox.all('option').each(function(option){
                if (option.get('selected') ||option.get('value') == 0) {
                    selectedJR[option.get('value')] = option.get('value');
                }
                option.remove();
            });
            ...
            for (index in data.results) {
                ...
                /* Job roles */
                fsJobroles  = dataSelector.jr;

                /* Add the new companies    */
                for (indexJR in fsJobroles) {
                    infoJR = fsJobroles[indexJR];
    ...
    // custom events
    Y.augment(search_jobrole, Y.EventTarget, null, null, {});
    // Initialise the user selector
    search_jobrole.init();
    // Store the user selector so that it can be retrieved
    this.search_jobroles[name] = search_jobrole;

    window.onbeforeunload = null;

    // Return the user selector
    return search_jobrole;
};