YUI.add('moodle-format_classroom-classroom_navigation', function (Y, NAME) {

/**
 * Course navigation block JS.
 *
 * This file contains the Course navigation block JS.
 *
 * @module moodle-format_classroom-classroom_navigation
 */

/**
 * This namespace will contain all of the contents of the classroom_navigation blocks
 * global classroom_navigation and settings.
 * @namespace M
 * @class format_classroom
 * @static
 */
M.format_classroom = M.format_classroom || {};
/**
 ...
 * @static
 */
M.format_classroom.expandablebranchcount = 1;
/**
 ...
 * @static
 */
M.format_classroom.courselimit = 20;
/**
 * Add new instance of classroom_navigation tree to tree collection
 *
 * @method init_add_tree
 * @static
 * @param {Object} properties
 */
M.format_classroom.init_add_tree = function (properties) {
 ...
 *
 * This event is delegated to branches in the classroom_navigation tree.
 * The on() method to subscribe allows specifying the desired trigger actions as JSON.
 *
 * @namespace M.format_classroom
 * @class ActionKey
 */
...
// Mappings for the different types of nodes coming from the classroom_navigation.
// Copied from lib/classroom_navigationlib.php classroom_navigation_node constants.
var NODETYPE = {
...
TREE.prototype = {
    ...
    initializer: function (config) {
        Y.log('Initialising classroom_navigation block tree', 'note', 'moodle-format_classroom');
        ...
        if (M.format_classroom.expandablebranchcount > 0) {
    ...
    toggleExpansion: function (e) {
        // First check if they managed to click on the li itself, then find the closest
    ...
    hideBranch: function (ele) {
        var list = ele.one("ul");
        list.setStyle('height', list.getStyle('height'));
...
Y.extend(TREE, Y.Base, TREE.prototype, {
    NAME: 'classroom_navigation-tree',
    ATTRS: {
        /**
        ...
        /**
         * If set to true nodes will be opened/closed in an accordion fashion.
         * @attribute accordion
         * @type Boolean
         */
        accordion: {
...
//            value : false
        ...
        /**
         * The classroom_navigation tree block instance.
         */
        instance: {
            value: false,
            setter: function (val) {
...
BRANCH.prototype = {
     ...
     * it into the classroom_navigation tree at the correct point.
     *
     * @method draw
     * @chainable
     * @param {Node} element
    ...
    ajaxLoad: function (e) {
        ...
        Y.log('Loading classroom_navigation branch via AJAX: ' + this.get('key'), 'note', 'moodle-format_classroom');
        ...
        // For siteadmin classroom_navigation get tree from getsiteadminbranch.php.
        ...
        try {
            ...
            if (object.children && object.children.length > 0) {
                ...
                if ((this.get('type') === NODETYPE.CATEGORY || this.get('type') === NODETYPE.ROOTNODE || this.get('type') === NODETYPE.MYCATEGORY)
                    && coursecount >= M.format_classroom.courselimit) {
                    this.addViewAllCoursesChild(this);
                }
                Y.log('AJAX loading complete.', 'note', 'moodle-format_classroom');
                // If this block can dock tell the dock to resize if required and check
                // the width on the dock panel in case it is presently in use.
            ...
            Y.log('AJAX loading complete but there were no children.', 'note', 'moodle-format_classroom');
        } catch (error) {
            if (outcome && outcome.status && outcome.status > 0) {
                // If we got here then there was an error parsing the result.
                Y.log('Error parsing AJAX response or adding branches to the classroom_navigation tree', 'error', 'moodle-format_classroom');
    ...
    addChild: function (branchobj) {
        ...
        if (branch.draw(this.getChildrenUL())) {
            ...
            if ((branch.get('type') === NODETYPE.CATEGORY || branch.get('type') === NODETYPE.MYCATEGORY)
                && count >= M.format_classroom.courselimit) {
    ...
    addViewAllCoursesChild: function (branch) {
        ...
        branch.addChild({
            ...
            link: url,
            haschildren: false,
            icon: {'pix': "i/classroom_navigationitem", 'component': 'moodle'}
        });
    }
};
Y.extend(BRANCH, Y.Base, BRANCH.prototype, {
    NAME: 'classroom_navigation-branch',
    ATTRS: {
        /**
        ...
        id: {
            ...
            getter: function (val) {
                if (val === '') {
                    val = 'expandable_branch_' + M.format_classroom.expandablebranchcount;
                    M.format_classroom.expandablebranchcount++;
                }
                return val;
            }
...
}, '@VERSION@', {"requires": ["base", "io-base", "node", "event-synthetic", "event-delegate", "json-parse", "transition"]});
