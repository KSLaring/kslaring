 ...
 */
class format_whitepaper_renderer extends plugin_renderer_base {
    ...
    public function display($course, $orphaned) {
        ...
        if ($orphaned) {
            if (!empty($modinfo->sections[1])) {
                $output .= $this->output->heading(get_string('orphaned', 'format_whitepaper'), 3, 'sectionname');
                $output .= $this->output->box(get_string('orphanedwarning', 'format_whitepaper'));
