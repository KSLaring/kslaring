...
class block_frikomport_renderer extends plugin_renderer_base {
    ...
    public function frikomport_tree(navigation_node $navigation) {
        ...
        $navigationattrs = array(
            ...
            'role' => 'tree');
        //$content = $this->navigation_node(array($navigation), $navigationattrs);
        // Use only the child nodes and avoid the root node.
        $nodes = $navigation->children;
        $nodesarray = array();
        foreach ($nodes as $node) {
            $nodesarray[] = $node;
        }
        $content = $this->navigation_node($nodesarray, $navigationattrs);
    ...
    protected function navigation_node($items, $attrs = array(), $expansionlimit = null, array $options = array(), $depth = 1) {
        ...
        foreach ($items as $item) {
            ...
            // There is no root element, so chack for depth = 0 to allow all subtrees to be collapsed.
            $pattr += $depth == 0 ? ['data-collapsible' => 'false'] : [];
    ...
    public function search_form(moodle_url $formtarget, $searchvalue) {
        $content = html_writer::start_tag('form',
            array('class' => 'frikomportsearchform', 'method' => 'get', 'action' => $formtarget, 'role' => 'search'));
        $content .= html_writer::start_tag('div');
        $content .= html_writer::tag('label', s(get_string('searchinfrikomport', 'block_frikomport')),
            array('for' => 'frikomportsearchquery', 'class' => 'accesshide'));
        $content .= html_writer::empty_tag('input',
            array('id' => 'frikomportsearchquery', 'type' => 'text', 'name' => 'frikomquery', 'value' => s($searchvalue)));
...
class block_frikomport_renderer_old extends plugin_renderer_base {
    public function frikomport_tree(navigation_node $navigation) {
