YUI.add('moodle-block_course_navigation-course_navigation', function (Y, NAME) {
 ...
 *
 * @module moodle-block_course_navigation-course_navigation
 */

/**
 * This namespace will contain all of the contents of the course_navigation blocks
 * global course_navigation and settings.
 * @namespace M
 * @class block_course_navigation
 * @static
 */
M.block_course_navigation = M.block_course_navigation || {};
/**
 ...
 * @static
 */
M.block_course_navigation.expandablebranchcount = 1;
/**
 ...
 * @static
 */
M.block_course_navigation.courselimit = 20;
/**
 * Add new instance of course_navigation tree to tree collection
 *
 * @method init_add_tree
 * @static
 * @param {Object} properties
 */
M.block_course_navigation.init_add_tree = function(properties) {
 ...
 *
 * This event is delegated to branches in the course_navigation tree.
 * The on() method to subscribe allows specifying the desired trigger actions as JSON.
 *
 * @namespace M.block_course_navigation
 * @class ActionKey
 */
...
// Mappings for the different types of nodes coming from the course_navigation.
// Copied from lib/course_navigationlib.php course_navigation_node constants.
var NODETYPE = {
...
TREE.prototype = {
    ...
    initializer : function(config) {
        Y.log('Initialising course_navigation block tree', 'note', 'moodle-block_course_navigation');
        ...
        for (var i in expansions) {
            ...
            M.block_course_navigation.expandablebranchcount++;
        ...
        if (M.block_course_navigation.expandablebranchcount > 0) {
...
Y.extend(TREE, Y.Base, TREE.prototype, {
    ...
    ATTRS : {
        ...
        /**
         * The course_navigation tree block instance.
         */
        instance : {
            value : false,
            setter : function(val) {
...
BRANCH.prototype = {
     ...
     * it into the course_navigation tree at the correct point.
     *
     * @method draw
     * @chainable
     * @param {Node} element
    ...
    ajaxLoad : function(e) {
        ...
        Y.log('Loading course_navigation branch via AJAX: '+this.get('key'), 'note', 'moodle-block_course_navigation');
        ...
        // For siteadmin course_navigation get tree from getsiteadminbranch.php.
        ...
        try {
            ...
            if (object.children && object.children.length > 0) {
                ...
                if ((this.get('type') === NODETYPE.CATEGORY || this.get('type') === NODETYPE.ROOTNODE || this.get('type') === NODETYPE.MYCATEGORY)
                    && coursecount >= M.block_course_navigation.courselimit) {
                    this.addViewAllCoursesChild(this);
                }
                Y.log('AJAX loading complete.', 'note', 'moodle-block_course_navigation');
                // If this block can dock tell the dock to resize if required and check
                // the width on the dock panel in case it is presently in use.
            ...
            Y.log('AJAX loading complete but there were no children.', 'note', 'moodle-block_course_navigation');
        } catch (error) {
            if (outcome && outcome.status && outcome.status > 0) {
                // If we got here then there was an error parsing the result.
                Y.log('Error parsing AJAX response or adding branches to the course_navigation tree', 'error', 'moodle-block_course_navigation');
    ...
    addChild : function(branchobj) {
        ...
        if (branch.draw(this.getChildrenUL())) {
            ...
            if ((branch.get('type') === NODETYPE.CATEGORY || branch.get('type') === NODETYPE.MYCATEGORY)
                && count >= M.block_course_navigation.courselimit) {
    ...
    addViewAllCoursesChild: function(branch) {
        ...
        branch.addChild({
            ...
            link : url,
            haschildren : false,
            icon : {'pix':"i/course_navigationitem",'component':'moodle'}
        });
    }
};
Y.extend(BRANCH, Y.Base, BRANCH.prototype, {
    NAME : 'course_navigation-branch',
    ATTRS : {
        /**
        ...
        id : {
            ...
            getter : function(val) {
                if (val === '') {
                    val = 'expandable_branch_'+M.block_course_navigation.expandablebranchcount;
                    M.block_course_navigation.expandablebranchcount++;
                }
                return val;
            }
