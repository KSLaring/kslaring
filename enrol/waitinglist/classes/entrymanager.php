...
class entrymanager  {
    ...
    */
	...
	/**
     * Adds a user to the confirmed list
     *
     ...
     */
     ...
     /*
	public function update($centry){
		global $DB;
		$centry->courseid=$this->courseid;
		$centry->waitinglistid=$this->waitinglist->id;
		$DB->update_record(self::CTABLE, $centry);
		
		//refresh our list
		$records =  $DB->get_records(self::CTABLE, array('courseid' => $this->courseid, 'waitinglistid'=>$this->waitinglist->id),'id ASC');
		if($records){
			$this->centries = $records;
		}
	...
	*/
	...
	/**
     * Adds an entry to the confirmed list
     *
     * @param stdclass queue object (db fields basically for confirmed table)
     * @return int the id of the queue item, or false if we somehow failed.
     */
     /*
	public function add($centry){
		global $DB;
		
		if ($this->is_on_list($centry->userid,$centry->methodtype)) {
            throw new \coding_exception('user is already on the confirmed list for this course and methodtype');
        } else {
            
            $centry->id = $DB->insert_record(self::CTABLE, $centry);
            
			if($centry->id){
				$this->centries[] =$centry;
				return $centry->id;
			}
			return false;
        }
	...
	*/
     ...
     */
     /*
	public function unconfirm_entry($centryid){
		global $DB;
		$centry = $this->get_centry($centryid);
		if(!$centry){return false;}
		
		$queueman= \enrol_waitinglist\queuemanager::get_by_course($this->courseid);
		$qentry = $queueman->get_qentry_by_userid($centry->userid);
		if($qentry){
			$qentry->allocseats = $qentry->allocseats - $centry->seats;
			$queueman->update($qentry);
		}else{
			$qentry = new \stdClass;
			$qentry->waitinglistid      = $centry->waitinglistid;
			$qentry->courseid       =$centry->courseid; 
			$qentry->userid       = $centry->userid; 
			$qentry->methodtype   = $centry->methodtype; 
			$qentry->seats = $centry->seats;
			$qentry->allocseats = 0;
			$qentry->queueno = 	0;
			$qentry->timecreated  = time();
			$qentry->timemodified = $qentry->timecreated;
			$queueman->add($qentry);
		}

		$ok = $DB->delete_records(self::CTABLE,array('id'=>$centryid));
		return $ok;
	}
*/
	...
	/**
     * Gets the total of users on our confirmed list
     *
     * @return int  users on the waiting list
     */
     
     /*
	public function get_seatcount($until_centryid=false){
		$seatcount = 0;
		if(!$this->centries){return 0;}
		foreach($this->centries as $centry){
			if($centry->id == $until_centryid){ 
				$seatcount += 1;
				break;
			}else{
				$seatcount += $centry->seats;
			}
		}
		return $seatcount;
	}
	*/
	...
	/**
     * GEts the total items on our confirmed list
     *
     * @return int  users on the confirmed list
     */
     /*
	public function get_entrycount(){
		return $this->centries ? count($this->centries) : 0;
	}
	*/
}
